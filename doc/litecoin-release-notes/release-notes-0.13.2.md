Metacoin Core version 0.13.2 is now available from:

  <https://download.metacoin.org/metacoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/metacoin-project/metacoin/issues>
...
updates will be released anymore. Without security updates, using a metacoin
...
ECDSA signatures inside Metacoin transactions now use validation using
...
Previous versions of Metacoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Metacoin Core was to raise the default minimum
relay fee.

Metacoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Metacoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running metacoind without having to do any manual configuration.
...
Metacoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Metacoin Core will relay transactions with
...
entirely in Metacoin Core 0.13, and it is at this time undecided whether the
...
Metacoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Metacoin Core automatically creates a hidden service to listen on, without
manual configuration. Metacoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Metacoin Core is listening, and
...
Metacoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Metacoin
Core to use floating fees. By default, Metacoin Core will use floating
fees.
...
At all times, Metacoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Metacoin Core will never create transactions paying less than
...
`metacoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /metacoinrpc>
        ...
        # optional bypass metacoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< metacoinrpc:<password>
    </Location>
...
Restarting metacoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in metacoin.conf
...
metacoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/metacoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Metacoin Core code base has
...
- `metacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `metacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Metacoin scripting system that in combination with
...
Metacoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Metacoin Core.
...
The two phases are distinct in the Metacoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Metacoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Metacoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Metacoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of metacoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of metacoins being spent is exactly
...
- **Increased security for multisig:** Metacoin addresses (both P2PKH addresses
  ...
  believed to be provided by Metacoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Metacoin's current ledger (called the UTXO set),
  ...
  note that Metacoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Metacoin users to individually opt-in to almost any change in the Metacoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Metacoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Metacoin
client to Metacoin Core 0.13.2 to signal for segwit support. When a
...
Since Metacoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `metacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `metacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
